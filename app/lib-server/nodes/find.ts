...
import { findDocuments, resolveMatchPushdown } from "./listData";
...
const findNode: NodeExecutor = {
    async run({ node, ctx, edges, nodes }) {
        ...
        try {
            ...
            // documents and filtering in memory — see resolveMatchPushdown's
            // own comment in ./listData.ts for exactly how (and why) it
            // renders Match's query the same way Match itself would.
            const pushdown = resolveMatchPushdown(nextNodeIds, nodes, edges, ctx);

            const documents = await findDocuments(list, pushdown?.field ?? "", pushdown?.operator ?? "equals", pushdown?.value ?? "");
