...
// this node and whose source is a "card" node, the very same edge that
// makes a Card chain into a View instead of rendering its own page (see
// CHAINS_INTO in lib/nodes/card.ts). When found, that Card's
// title/subtitle/body templates become this List View's item template
// instead of the plain field dump in defaultText above.
function findChainedCard(node: IModuleNode, nodes: IModuleNode[], edges: IModuleEdge[]): IModuleNode | null {
...
function findChainedLink(node: IModuleNode, nodes: IModuleNode[], edges: IModuleEdge[]): IModuleNode | null {
...
// Shared by listViewNode.run() (standalone-page use) and View's
// "listView" block (lib/nodes/view.ts) — same split as Table's
// resolveRows in ./table.
export async function resolveListItems(node: IModuleNode, nodes: IModuleNode[], edges: IModuleEdge[], ctx: NodeContext): Promise<{ fields: TableField[]; items: ListViewItem[] }> {
...
const listViewNode: NodeExecutor = {
