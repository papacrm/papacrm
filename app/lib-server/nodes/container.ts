...
function parseBlocks(node: IModuleNode): Record<string, any>[] {
    try {
...
async function resolveBlock(block: Record<string, any>, node: IModuleNode, ctx: NodeContext) {
    if (block.type === "table") {
        const { fields, documents } = await loadListTableData(String(block.listId ?? "").trim(), ctx.moduleId);
    ...
    if (block.type === "form") {
        return {
            ...
            // Same purpose as Input Form's own nodeId: lets the engine tell
