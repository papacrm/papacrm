...
// — https://nukejs.com/docs/state-management). This node never mutates
...
// server-rendering one: alongside the write, this node also queues a
...
//     numberInput work: those nodes render their field with
//     id={name}, so id === store key === live prefill.
//   - the JSON debug block a directly-chained View renders (tagged with
//     data-state-debug="<this node's id>" — see lib/nodes/view.ts) — this
...
//     app (e.g. a hydrated dashboard page) sees whatever this node wrote
//     — and this node, in turn, picks up whatever a real
...
(function () {
  try {
    ...
    // State node is chained into — a page render can only ever guess at
...
function resolvePatch(mapping: Record<string, string>, ctx: NodeContext): Record<string, string> {
...
const stateNode: NodeExecutor = {
    run({ node, ctx, edges }) {
        ...
        if (store && Object.keys(mapping).length) {
            ...
            // directly after this node (View, or one of the Input nodes)
            // — see the doc on ctx.stateValues in ./types.ts. Overwrite
            // rather than merge: only the most recently run State node's
            // data is live.
            ctx.stateValues = { nodeId: node.id, data: patch };
        } else {
            // Nothing mapped — don't leave a stale hand-off from an
            // earlier State node lying around for this node's own
