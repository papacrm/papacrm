...
// Stores data from the previous node into a NukeJS store
...
// Mapper it never folds its resolved values back into the shared module
// context for arbitrary downstream {{templating}}.
//
// What it *does* do is hand its resolved values to whatever node is wired
// directly after it, for that one node to use:
//   - State → View: the View auto-displays the values as formatted JSON at
//     the top of the page, for debugging.
//   - State → (Text/Checkbox/Textarea/Number) Input: that input node shows
...
// elsewhere in the app reads back whatever this node wrote.
const stateNode: ModuleNodeDef = {
    type: "state",
    label: "State",
    description:
        "Stores data into a NukeJS store in the visitor's browser (memory or localStorage). Hands its mapped values to a directly-chained View (as JSON) or Input node (as its value).",
    color: "#7c3aed",
    kind: "action",
    fields: [
