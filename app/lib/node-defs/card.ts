...
// chainable, same idea as View — see lib/nodes/view.ts's long comment on
...
//     templates to be used as that node's per-item layout instead of a
//     plain field dump. See lib/nodes/listView.ts (resolveListItems) and
//     lib/nodes/card.ts (resolveCardItems) on the server.
//   - nowhere — Card renders its own page, one card per row, same as
//     Table renders its own page of rows when nothing's downstream.
const cardNode: ModuleNodeDef = {
    type: "card",
    label: "Card",
    description: "Shows the previous node's data as cards — chain it into a View to embed a card grid, or into a List View to use it as that list's item template",
    color: "#be185d",
    kind: "action",
    fields: [
    ...
    inspectorNote: () => ({
        label: "Tip",
        value: "Feed this from a Query node, an Input Form, or a webhook's own query string/POST body, same as Table — each row becomes one card.",
    }),
};
