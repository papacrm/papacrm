...
// View is a page-builder node: whatever Menu / Tabs / Navbar / Footer /
// Table / Input Form / Function / another View is connected into it (drag
// from that node's output dot onto this node) shows up as a placeable
// block in this node's inspector — see the "Layout" section
// ModuleEditor.tsx renders specially for selected View nodes, right
...
// connected node's id — there's no field for it here because the layout
...
// lib/nodes/view.ts and ModuleEditor.tsx's Layout section, neither of
...
// viewNode.run() in lib/nodes/view.ts, which follows the chain instead of
// finishing at a View that has somewhere else to go.
//
// Wiring a View into anything *other* than a View — most usefully a Call
// node — means something different: render this View's own page normally,
// but instead of answering with it directly, hand it off. Wire it into a
// Call node that calls a shared layout's Function (that Function chained
...
// ViewBlock in lib/nodes/view.ts and callNode.run() in lib/nodes/call.ts.
// That's how one layout gets reused across many different pages, even
// pages that live in entirely different modules.
const viewNode: ModuleNodeDef = {
    type: "view",
    label: "View",
    ...
    color: "#7c2d12",
    kind: "action",
    fields: [{ key: "title", label: "Page title — use {{field}} for data from an earlier node", kind: "text", placeholder: "{{title}}" }],
