...
export default function InputFormPage({ title, submitLabel, fields, nodeId }: InputFormPageProps) {
    useHtml({ title });

    return (
        <main data-webhook-page className="mx-auto flex max-w-md flex-col gap-6 px-6 py-16">
            <h1 className="text-2xl font-semibold">{title}</h1>
            <WebhookInputForm fields={fields} submitLabel={submitLabel} nodeId={nodeId} />
        </main>
    );
}